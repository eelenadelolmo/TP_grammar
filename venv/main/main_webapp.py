...
# Input directory
dir_original = 'in'
shutil.rmtree(dir_original, ignore_errors=True)
...
def annotate():
    ...
    # Rules classified as 'main' select the main clause and rewrite the graph
    # Rules classified as 'in_main' Rules match the main clause with the rewriting rules applied
    # The grammars file must contain rules separated by '####'
    # and begin with the type ('main' or 'in_main') and the identifier of every rule in the first two non-blank lines for every rule

    n_gram = 0
    n_exceptions = 0
    n_processed = 0

    grs_list_dict_main = dict()
    for grs in grs_list:
        type = grs.strip().split('\n')[0]
        grs = ' '.join(grs.strip().split('\n')[2:])
        grs_list_dict_main[grs] = type


    print("cacaaa", grs_list_dict_main)



    for grs in grs_list_dict_main:
